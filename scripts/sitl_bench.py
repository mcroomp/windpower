#!/usr/bin/env python3
"""
sitl_bench.py -- Interactive SITL bench session for calibrate.
...
This lets calibrate connect over tcp:localhost:5760 from the Windows host
...
Usage (inside a rawes-sim container with MAVLink port 5760 published):
    /rawes/.venv/bin/python3 /rawes/scripts/sitl_bench.py [--omega-rotor RAD_S]

Connect from Windows host (second terminal):
    python -m calibrate --port sitl
"""
...
_SIM_DIR     = _REPO_ROOT / "simulation"                # simulation/
_SITL_DIR    = _REPO_ROOT / "tests" / "sitl"            # tests/sitl/

from tests.sitl.stack_utils import (   # noqa: E402
    ParamSetup,
    _launch_sitl,
    _prime_sitl_eeprom,
...
_BOOT_PARAMS: dict[str, float] = {
    ...
    # Lua scripting
    "SCR_ENABLE":         1,
    "RAWES_MODE":         0,     # MODE_NONE at boot; calibrate sets mode
    # Arming — skip all prearm checks so force-arm from calibrate works
...
def main() -> None:
    ...
    print("    python -m calibrate --port sitl")
    print()
    print(f"  Logs: {tmp}/")
