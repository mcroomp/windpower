...
- **Temporary/working files must live under `tmp/` at repo root.** Put scratch outputs, ad-hoc notes, transient CSV/JSON/TXT dumps, and throwaway helper files in `tmp/` only; do not create them in the repo root or source directories.
