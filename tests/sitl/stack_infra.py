...
_SCRIPTS_DIR = _SIM_DIR.parent / "scripts"  # scripts/ — all Lua scripts


def _install_lua_scripts(*names: str) -> None:
    """
    Copy the named Lua scripts from scripts/ to SITL's /ardupilot/scripts/.
...
def get_arducopter_crash_info(ctx) -> str:
    ...
    try:
        ...
        from analysis.analyse_run import parse_arducopter, RunReport  # type: ignore[import]
        rpt = RunReport()
...
def _torque_stack(
    ...
    *,
    ...
    install_scripts        : tuple of Lua script names to install from scripts/
