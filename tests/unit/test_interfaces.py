...
from simulation.sitl_interface import SIM_CLOCK_HZ, SITLInterface
...
def _make_servo_pkt_16(pwm: list[int], frame_rate: int = 400, frame_count: int = 1) -> bytes:
    from simulation.sitl_interface import _SERVO_FMT_16, _SERVO_MAGIC_16
...
def test_sitl_interface_recv_servos_32ch_reads_first_16():
    from simulation.sitl_interface import _SERVO_FMT_32, _SERVO_MAGIC_32
    recv_port = _get_free_udp_port()
    interface = SITLInterface(recv_port=recv_port, watchdog_timeout=0.05)
    interface.bind()
    sender = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
