...
MockArdupilot pumping Python mode's bz_goal before the rate-cmd is built.  Checks whether the
pendulum mode that wouldn't settle in warmup_gain_sweep is damped by it.

Run:
    .venv/Scripts/python.exe tests/oneoff/warmup_with_damping.py
"""
import math
import sys
...
from simulation.controller import (HeliCyclicController, compute_bz_altitude_hold,
                              compute_rate_cmd, damp_bz_eq_lateral)
from simulation.mediator import TetherModel
...
def run(kd_lat, t_total, R0, pos0, omega_spin, rest_length, trim,
    ...
    for step in range(n):
        ...
        if step % _AP_EVERY == 0:
            ...
            obs = runner.observe()
            # Advance the AP to update elevation state, but use the
            # collective only.
            col_held, _, _ = ap.controller_step(obs, dt=1.0 / MockArdupilot.AP_HZ)
