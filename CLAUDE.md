...
**Simtest status (13 simtests, 11 PASS):** 9 Python-only simtests pass. `test_steady_flight_lua`, `test_pump_cycle_lua`, and `test_pump_cycle_unified` (both variants) pass. `test_landing` and `test_landing_lua` failing (winch control during descent — under investigation). `test_kinematic_transition` pre-existing failure. `test_pump_cycle_unified.py` uses `TensionCommand`/`TensionApController` architecture + `VirtualComms` comms boundary. Landing uses unified architecture: `LandingGroundController` (10 Hz) → `LandingCommand` → `LandingApController` (400 Hz) + `WinchController` (400 Hz). Old `LandingPlanner` deleted.
...
- **TensionPI runs at 400 Hz (physics loop); DeschutterPlanner runs at 10 Hz.** Planner provides `tension_setpoint` and `col_min_rad` to TensionPI at 10 Hz; TensionPI adjusts collective every physics step. `planner.step()` returns `col_min_rad` key for this purpose. Correct pattern: `if i % planner_every == 0: pump_cmd = planner.step(state, DT_PLANNER); tension_pi.setpoint = ...; tension_pi.coll_min = pump_cmd["col_min_rad"]`. Then every step: `col = tension_pi.update(tension_now, DT)`. **Note:** TensionPI runs inside rawes.lua for Lua simtests (`test_pump_cycle_lua`). The Python `test_pump_cycle_unified.py` uses `TensionCommand`/`TensionApController` instead — see below.
...
- **`AcroControllerSitl` (use_servo=True, 25 ms servo lag)** is baked into `PhysicsRunner` and always active for all simtests. No per-test configuration needed.
