...
- **rawes.lua landing (SCR_USER6=4):** captures body_z on `ahrs:healthy()`; VZ descent controller (`col_cmd = COL_CRUISE + KP_VZ*(vz_actual - VZ_LAND_SP)`, VZ_LAND_SP=0.5 m/s); `final_drop` (collective→0) triggered by ground planner sending `NAMED_VALUE_FLOAT("RAWES_SUB", 1)` (LAND_FINAL_DROP). Fixture: `acro_armed_landing_lua` (`kinematic_vel_ramp_s=20` so hub exits kinematic at vel=0 — eliminates linear tether jolt).
...
| M3 Step 1b — test_landing.py + test_landing_lua (simtest) | failing: winch control during descent — under investigation | descent slack=0, floor hit, anchor_dist < 20 m |
