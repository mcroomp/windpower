...
- **rawes.lua SCR_USER6 modes (valid values 0,1,2,4,5):** 0=none (passive: no RC overrides, logs every 5 s + any NV message), 1=steady (cyclic orbit-tracking, 50 Hz), 2=yaw_lua (counter-torque yaw trim only, Ch9/SERVO9, 100 Hz), 3=reserved, 4=landing (cyclic + VZ descent + auto final_drop, 50 Hz), 5=pumping (De Schutter pumping cycle, 50 Hz). Modes 4+5 own Ch3 entirely. **RAWES_ARM:** send `NAMED_VALUE_FLOAT("RAWES_ARM", ms)` to arm the vehicle and start a disarm countdown of `ms` milliseconds; re-sending refreshes the timer; Lua disarms and sends a STATUSTEXT when the countdown expires. Works in any mode. Default state is unarmed. **Yaw trim alongside any flight mode:** send `NAMED_VALUE_FLOAT("RAWES_YAW", 1)` to enable, 0 to disable; cleared on every mode change. **Substates** (landing: 0=descend/1=final_drop; pumping: 0=hold/1=reel_out/2=transition/3=reel_in/4=transition_back) are delivered via `NAMED_VALUE_FLOAT("RAWES_SUB", N)` — never encoded in SCR_USER6. `_nv_floats` resets to `{}` on every mode change. Constants in `simulation/rawes_modes.py`; send via `gcs.send_named_float("RAWES_SUB", N)` or `sim.send_named_float("RAWES_SUB", N)` (unit harness).
