...
```
I_spin = N × I_blade + I_hub_shell
       = 4 × 0.904 + 0.326 ≈ 3.94 kg·m²
```

where each blade contributes `I_blade = m_blade × (R² + R·r_root + r_root²)/3 = 0.35 × 7.75/3 = 0.904 kg·m²` (uniform density, radial distribution).
...
- **Coleman skewed-wake induction** for non-uniform induction at high disk tilt (ξ > 20°)
...
The disk normal (body-z) is tilted 29° from the wind direction. This is the *autorotation operating angle*: tilting the disk into the wind provides axial inflow that drives rotor spin, while the remaining in-plane wind component generates the tangential force that sustains orbit.
...
```
H = I_spin × ω_eq = 3.94 × 19.3 ≈ 76 kg·m²/s
```
...
The hub traces a quasi-circular orbit on the surface of a tether-length sphere. With wind along East (Y-axis in NED), the orbit plane is approximately vertical and perpendicular to the wind, forming a cone of half-angle θ around the wind axis:

```
θ = arccos(tether_dir · wind_hat) ≈ arccos(0.950) ≈ 18°
```

The orbital radius (arc perpendicular to the wind axis) is:

```
r_orbit ≈ L × sin(θ) = 50 × sin(18°) ≈ 15.6 m
```

The orbital period is approximately **60 s** at 10 m/s wind and 50 m tether length, giving:

```
ω_orbit = 2π / T_orbit ≈ 0.105 rad/s (world frame)
```

In the body frame of the tilted disk (67° from vertical), this azimuthal rotation projects onto all three body axes, producing a persistent angular rate disturbance of approximately **0.2–0.3 rad/s** in roll and pitch.
...
1. The disk is tilted at ξ ≈ 29° from the wind, producing a net thrust vector misaligned with the tether.
...
The disk tilt angle ξ changes between reel-out and reel-in phases, altering both the orbital geometry and the BEM operating point. Table 2 summarises key operating conditions at 10 m/s wind:

| Phase | ξ [°] | ω_eq [rad/s] | Fz (col=−0.20) [N] | Fz (col=0.00) [N] | Fz (col=+0.10) [N] |
|-------|--------|-------------|---------------------|--------------------|--------------------|
...
**Reel-out (ξ = 30–50°):** The disk is tilted strongly into the wind. Axial inflow velocity v_axial = v_wind × cos(ξ) = 8.2 m/s at ξ=35° gives strong thrust. The thrust vector lies nearly along the tether direction, producing high tether tension (~200 N) and therefore high reel-out power.

**Nominal reel-in (ξ = 55°):** At col = −0.10 rad, Fz ≈ 72 N > 49 N (hub weight supported). At col = −0.20, Fz < 0 — the disk produces downforce. The zero-thrust collective shifts to approximately −0.228 rad at ξ=55° vs. −0.34 rad at ξ=29°. **This shift must be reflected in the collective floor used by the tension PI controller**, or the hub will descend during reel-in.

**High-tilt reel-in (ξ = 80°):** Altitude maintenance requires positive collective (col = +0.10 rad), outside the normal range [−0.28, 0.00] rad. High tilt yields ξ=80° net energy +24% compared to ξ=55° baseline, due to dramatically lower reel-in tension (~58 N vs ~86 N).
...
With I_spin = 3.94 kg·m² and ω_eq = 19.3 rad/s, the gyroscopic couple for a small cyclic torque M_cyc is:

```
ω_precession = M_cyc / H = M_cyc / 76 rad/s
```

For M_cyc ≈ 50 N·m (typical cyclic torque), ω_precession ≈ 0.66 rad/s — faster than any useful planner rate.

However, simulation shows that with a strong orbital angular damping term (k_ang = 50 N·m·s/rad), the gyroscopic cross-coupling torque `ω_orbital × H` remains small because orbital angular rates are strongly damped (damping time constant τ = I_body/k_ang = 5.0/50 = 0.1 s). Under this condition, gyroscopic coupling at I_spin ≈ 3.94 kg·m² does **not** destabilise the orbit even at zero swashplate phase compensation.

### 5.2 Phase Sweep Results

Simulating the full orbit for 30 s with swashplate phase compensation swept from 0° to 180° in 30° steps (at I_spin = 3.94 kg·m²):

- Orbit remains stable (no floor hits, drift < 200 m, min altitude > 2 m) for all phases 0°–180° under the current angular damping regime.
- This is consistent with a hardware phase compensation range of **60–120°**, matching the theoretical prediction of 90° for a counter-clockwise rotor.

The stable phase window matters for hardware tuning: when setting `H_PHANG` in ArduPilot, any value in the 60–120° range should produce stable closed-loop response. The exact optimal value is determined by a flight step-response test.
...
With the high-tilt strategy (ξ = 80° reel-in, col_max extended to +0.10 rad), net energy improves to approximately +1735 J/cycle (+24%), with reel-in tension dropping to ~58 N.

The energy asymmetry arises entirely from orbit geometry: during reel-out the disk normal is aligned with the tether (ξ ≈ 30–35°), maximising the thrust-along-tether component; during reel-in the disk normal rotates toward vertical (ξ = 55–80°), redirecting thrust upward to support altitude while minimising tether resistance.
...
**Problem:** At orbital equilibrium the RAWES hub sits at ~67° from vertical (NED down). Standard attitude estimation (e.g. ArduPilot EKF3) interprets this as ~67° roll/pitch error from level and immediately commands full cyclic deflection, crashing the hub within 1–2 s.
...
**Identification:** This bias is directly measurable: with the hub in stable orbit and zero RC stick input, the I-term on the roll and pitch rate PIDs should grow monotonically. The timescale before saturation is approximately `tilt_max / (I_gain × ω_orbit) ≈ 0.3 / (0.03 × 0.2) ≈ 50 s` at default ArduPilot gains.
...
**Problem:** A naive attitude controller (PD on body-z error relative to the *initial* tether direction) accumulates phase lag as the hub orbits. Within one quarter-orbit (~15 s) the tether direction has rotated ~90° azimuthally, making the initial reference useless and commanding large corrective cyclic.
...
**Problem:** During the reel-out → reel-in transition, the planner commands the disk to tilt from ξ ≈ 35° to ξ ≈ 55–80°. If this command is applied instantaneously, the gyroscopic response produces a large transient cyclic moment that destabilises the orbit.

**Analysis:** The disk tilt rate achievable through gyroscopic precession is ω_tilt = M_cyc / H. The *maximum possible* M_cyc comes from full aerodynamic cyclic authority across all four blades at equilibrium spin, giving a theoretical upper bound:

```
ω_tilt_max = M_cyc_max / H ≈ 20 rad/s
```
...
**Implication for cycle timing:** The minimum transition time from ξ=35° to ξ=80° is:

```
Δξ / slew_rate = (80° − 35°) × π/180 / 0.40 ≈ 2.0 s
```
...
At equilibrium: mean East = 47.5 m, mean North = 13.9 m → estimated wind azimuth = arctan(47.5/13.9) ≈ 74° from North = East-northeast. True wind is due East. The small bias (16°) is the azimuthal equilibrium offset due to rotor angular momentum and asymmetric aerodynamics.

**Accuracy:** This estimator requires one full orbit period (~60 s) to converge and provides wind direction to within ~20° without any dedicated wind sensor — useful for initialising the reel-in disk tilt quaternion before a dedicated anemometer measurement is available.

**In-plane wind speed** is simultaneously estimable from rotor spin rate via the autorotation torque balance:

```
v_inplane ≈ ω_spin² × K_drag / K_drive
```
...
De Schutter et al. [5] characterise the pumping cycle through OCP and report reel-out tilt angles of 30–50° and reel-in tilt >70°, which our BEM simulation confirms. An earlier poster [11] introduced the inertia-based pumping cycle concept but without closed-loop controller analysis. Neither work addresses:
...
2. **Steady-state BEM:** No dynamic inflow, no wake memory. The Coleman skewed-wake correction handles the most important non-uniformity but is not valid above ξ ≈ 85°.
...
5. **10 m/s wind only:** All results are at a single design-point wind speed. Power output scales approximately as v_wind³; orbit period and radius scale differently, requiring sweeps for variable-wind operation.
...
| Gyro phase comp | H_PHANG = 60–120° (exact value from step-response test) |

The flight mode choice follows from requirement (2): STABILIZE mode commands absolute NED attitude (roll=0=level), which fights the 65° orbital equilibrium. ACRO mode only damps angular rates toward commanded rates, making it compatible with orbit tracking once the I-term is disabled.
