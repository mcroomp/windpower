...
def test_not_ready_initially():
    ...
    assert est.wind_dir_enu is None
...
def test_wind_dir_eastward():
    """Hub consistently east of anchor → wind_dir_enu points east."""
    est = WindEstimator(min_samples=10)
    for i in range(20):
        angle = i * 2 * np.pi / 20
        # Hub orbits at radius ~48 m, east of anchor (mean x ≈ 46, mean y ≈ 0)
    ...
    assert abs(d[1]) < 0.1
    assert abs(d[2]) < 1e-9   # horizontal: z = 0


def test_wind_dir_northward():
    """Hub consistently north of anchor → wind_dir_enu points north."""
    est = WindEstimator(min_samples=10)
    for i in range(20):
        angle = i * 2 * np.pi / 20
...
def test_deschutter_updates_reel_in_q_when_estimator_ready():
    ...
    # Feed estimator with hub positions indicating wind from north
    # mean pos should be north of anchor → wind_dir = north
    for i in range(10):
        angle = i * 2 * np.pi / 10
    ...
    # The new body_z from q_after should point roughly north+up (northward wind)
    body_z = quat_apply(q_after, np.array([0.0, 0.0, 1.0]))
    assert body_z[1] > 0.3, f"expected northward tilt in body_z, got {body_z}"
